...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        while ptr < png_length {
            if ptr + 12 > png_length {
                return Err(PngError::NextChunkTooShort);
            }

            let mut length_bytes = [0u8; 4];
            length_bytes.copy_from_slice(&value[ptr..ptr + 4]);
            let length = u32::from_be_bytes(length_bytes);
            let chunk_length = length as usize + 12;

            if ptr + chunk_length > png_length {
